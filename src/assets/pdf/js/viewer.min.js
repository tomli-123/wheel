...
PDFJS.workerSrc = '../pdf/js/pdf.worker.min.js';
PDFJS.cMapUrl = './cmaps/';
  ...
  PDFPresentationMode.prototype = {
    ...
    mouseScroll: function PDFPresentationMode_mouseScroll(delta) {
      ...
      if(Math.abs(this.mouseScrollDelta) >= PAGE_SWITCH_THRESHOLD) {
        var pageSwitchDirection = (this.mouseScrollDelta > 0) ? PageSwitchDirection.UP : PageSwitchDirection.DOWN;
        var page = PDFViewerApplication.page;
        this._resetMouseScrollState();
        if((page === 1 && pageSwitchDirection === PageSwitchDirection.UP) || (page === PDFViewerApplication.pagesCount && pageSwitchDirection === PageSwitchDirection.DOWN)) {
          return
        }
  ...
  PDFPageView.prototype = {
    ...
    draw: function PDFPageView_draw() {
      ...
      this.renderTask.promise.then(function pdfPageRenderCallback() {
        ...
        if(textLayer) {
          self.pdfPage.getTextContent().then(function textContentResolved(textContent) {
    ...
    scrollPageIntoView: function PDFViewer_scrollPageIntoView(pageNumber, dest) {
      ...
      switch(dest[1].name) {
        ...
        case 'FitR':
          ...
          widthScale = (viewerContainer.clientWidth - hPadding) / width / CSS_UNITS;
          heightScale = (viewerContainer.clientHeight - vPadding) / height / CSS_UNITS;
...
var PDFViewerApplication = {
  ...
  open: function pdfViewOpen(file, scale, password, pdfDataRangeTransport, args) {
    if(this.pdfDocument) {
      Preferences.reload()
    }
    this.close();
        ...
        if(zoomArg.indexOf('Fit') === -1) {
          ...
          if(zoomArg === 'Fit' || zoomArg === 'FitB') {
            ...
            if(zoomArgs.length !== 5) {
              console.error('pdfViewSetHash: ' + 'Not enough parameters for \'FitR\'.')
            } else {
              dest = [null, {
                name: zoomArg
            ...
            console.error('pdfViewSetHash: \'' + zoomArg + '\' is not a valid zoom value.')
          }
        }
      }
      if(dest) {
  ...
  beforePrint: function pdfViewSetupBeforePrint() {
    ...
    this.pageStyleSheet.textContent = '@supports ((size:A4) and (size:1pt 1pt)) {' + '@page { size: ' + pageSize.width + 'pt ' + pageSize.height + 'pt;}' + '#printContainer {height:100%}' + '#printContainer > div {width:100% !important;height:100% !important;}' + '}';
...
window.addEventListener('resize', function webViewerResize(evt) {
  if(PDFViewerApplication.initialized && (document.getElementById('pageAutoOption').selected || document.getElementById('pageFitOption').selected || document.getElementById('pageWidthOption').selected)) {
...
window.addEventListener('scalechange', function scalechange(evt) {
  ...
  if(!PDFViewerApplication.updateScaleControls && (document.getElementById('pageAutoOption').selected || document.getElementById('pageActualOption').selected || document.getElementById('pageFitOption').selected || document.getElementById('pageWidthOption').selected)) {
    updateViewarea();
    return
  }
  if(evt.presetValue) {
  ...
  if(!predefinedValueFound) {
    ...
    customScaleOption.textContent = mozL10n.get('page_scale_percent', {
      scale: customScale
    }, '{{scale}}%');
...
window.addEventListener('keydown', function keydown(evt) {
  ...
  if(cmd === 1 || cmd === 8 || cmd === 5 || cmd === 12) {
    switch(evt.keyCode) {
      ...
      case 71:
        if(!PDFViewerApplication.supportsIntegratedFind) {
          PDFViewerApplication.findBar.dispatchEvent('again', cmd === 5 || cmd === 12);
          handled = true
        }
        break;
      case 61:
  ...
  if(cmd === 0) {
    switch(evt.keyCode) {
      ...
      case 33:
      case 8:
        if(!isViewerInPresentationMode && PDFViewerApplication.currentScaleValue !== 'page-fit') {
          break
        }
      case 37:
